...
internal void *arena_push_no_zero(Arena *arena, u64 size)
{
  ...
  if (current->growing) {
    u64 next_chunk = ALIGN_POW2(current->chunk_pos, current->align);
    u64 next_chunk_end = next_chunk + size;

    if (next_chunk_end > current->chunk_cap) {
      u64 new_chunk_size = size + ARENA_HEADER_SIZE;
...

  {
    ...
    u64 next_chunk_start = ALIGN_POW2(current->chunk_pos, current->align);
    u64 next_chunk_end = next_chunk_start + size;

    if (next_chunk_end <= current->chunk_cap) {
      if (next_chunk_end > current->commit_pos) {
        u64 next_commit_pos = ALIGN_POW2(next_chunk_end, ARENA_DEFAULT_COMMIT);

        // @Note: Ensure that we won't _overcommit_, also means that we can't align.
        u64 next_commit_min = MIN(next_commit_pos, current->chunk_cap);

        u64 next_commit_size = next_commit_min - current->commit_pos;
...
internal void arena_pop_to(Arena *arena, u64 pos)
{
  ...
  u64 pos_clamp = MAX(pos, ARENA_HEADER_SIZE);
  ...
  u64 rel_chunk_pos = MAX(pos_clamp - current->base_pos, ARENA_HEADER_SIZE);
