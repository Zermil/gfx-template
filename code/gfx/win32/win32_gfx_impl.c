  ...
  switch (msg) {
    ...
    case WM_DROPFILES: {
      ...
      for (u32 i = 0; i < drop_files.count; ++i) {
        ...
        drop_files.files[i].name.size = (u64) size;
        drop_files.files[i].name.data = arena_push_array(win32_arena, u8, (u64) size);
...
internal GFX_Window *gfx_win32_opaque_from_handle(HWND handle)
{
  ...
  if (!gfx_is_init()) {
    ...
    if (handle != 0) {
      ...
      for (u64 i = 0; i < ARRAY_SIZE(win32_windows); ++i) {
