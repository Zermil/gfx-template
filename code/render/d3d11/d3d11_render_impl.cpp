...
"  output.quad_half = (input.quad.zw - input.quad.xy)/2.0f;\n"
"\n"
"  // @Note: Treat rect bigger than actually, makes for a nicer smoothstep.\n"
"  output.quad_pos = output.quad_center + input.pos*(output.quad_half + input.radius);\n"
"\n"
...
"  float sdf = sdf_rect(coord, input.quad_half, input.radius);\n"
"  float s = 1.0f - smoothstep(0.0f, 2.0f, sdf);\n"
"\n"
"  // @Note: We flip because we submit the colours in rgba format, windows flips them\n"
"  // again, it's a whole mess, ABGR is correct.\n"
  ...
  if (!error) {
    ...
    d3d11_state.context->Map(d3d11_state.buffer[I_BUFFER], 0, D3D11_MAP_WRITE_DISCARD, 0, &vertex_resource);
    {
      ...
      for (R_Quad_Node *node = draw_data; node != 0; node = node->next) {
        u64 bytes = node->count * sizeof(R_Quad);
